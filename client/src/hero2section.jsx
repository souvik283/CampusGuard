    ...
    <>
      ...
      <div className="ar">
        ...
        <div className="ac">
          <div className="lr">
            <ShieldIcon size={28} />
            <span className="lt">TruOffer</span>
          </div>
          <div className="tr">
            <button
              className={`tb ${tab === "login" ? "on" : "off"}`}
              onClick={() => switchTab("login")}
            >
              Sign In
            </button>
            <button
              className={`tb ${tab === "register" ? "on" : "off"}`}
...
export default function TruOfferApp() {
  ...
  if (page === "login" || page === "register")
    return <AuthPage initialTab={page} onBack={() => setPage("landing")} />;
  if (page === "verify")
    return <VerifyDashboard onBack={() => setPage("landing")} />;
  return (
    <LandingPage
      onLogin={() => setPage("login")}
      onGetStarted={() => setPage("verify")}
    />
  );
}
